...
## 7.一些题目设计的思考
为了反映切实的思考，题目应当无任何程度随机性，如剪刀石头布的题目，由于轮换后等效，认为是随机的题目（解决方法可以是给石头赢剪刀，剪刀赢布，布赢石头分别给不一样的权重）
每题的期望可以设置为5分左右（不限制，只是为了避免出现一题加减几十分影响太大的情况）
避免阅读理解，题目文本应该简洁，可分为题目文本和计分规则两部分（目前，也可不分）
考虑到题目适用的人数和期望选项几人选择再思考选项的数量（选择题性质的，人数较少不应该有太多选项）

### 两人题的思考
两人题很容易弄成某种剪刀石头布（概念意义上的互相克制，并非真正的随机）或是围绕某个数字选项之间的争夺，不建议专门设计两人的题目
