...
public class JwtProperties {
    ...
    public void validateSecret() {
        ...
        if (secret.contains("dev-only-secret")) {
            log.warn(" 警告：正在使用开发环境默认JWT密钥！");
            log.warn(" 这仅适用于本地开发，生产环境必须设置 JWT_SECRET 环境变量！");
            log.warn(" 建议使用命令生成强密钥: openssl rand -base64 32");
            return; // 开发环境允许使用默认密钥
        }

        // 检查密钥强度（至少32字节 = 256位）
        if (secret.getBytes().length < 32) {
            log.warn(" JWT密钥长度不足32字节，建议使用更强的密钥（256位以上）");
        }
